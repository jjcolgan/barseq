...
'percent perfect barcodes improves model compared to just fitness'

'categorical day also improves the aic relative to just fitness'

'numeric day looks much better relative to categorical day'
...
for (l in 1:length(loci)) {
  ...
  input$tissueNumeric = 0
  input$tissueNumeric[input$tissue == 'dj'] = 1

  # Fit the model and capture the summary
  glm_fit = glm(data = input, formula = fitness ~ tissue+dayNumeric+percentPerfectBarcode+millionBases)

  # Extract the p-value for fitness from the model summary
  glm_summary = summary(glm_fit)
  pvalue = coef(glm_summary)[2, 4]  # Coefficient for 'fitness' is typically in row 2
  aic = glm_summary$aic
  # Store the gene and p-value in the data frame
  modelStats = rbind(modelStats, data.frame(gene = loci[l], pvalue = pvalue, aic = aic))
}
write_tsv(modelStats, 'linear models/lmTissueComparisionsAllTimePoints/tissuePercentNumericDayMillionBasesAsPredictorModelStats.tsv')
