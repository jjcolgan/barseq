...
# Join annotations into resMaaslinNumericDayDj
...
title=briteAnnotatedTemporalSignificantdj%>%
  filter(locusId == 'BBR_RS10465')%>%
  .$kofamFunction

plotIn %>%
  filter(locusId == 'BBR_RS10465')%>%
  left_join(briteAnnotatedTemporalSignificantdj, by = "locusId")%>%
...
title=briteAnnotatedTemporalSignificantdj%>%
  filter(locusId == 'BBR_RS10670')%>%
  .$kofamFunction

plotIn %>%
  filter(locusId == 'BBR_RS10670')%>%
  left_join(briteAnnotatedTemporalSignificantdj, by = "locusId")%>%
  filter(kofamAccession != 'K12267')%>%
  ggplot(aes(x = dayNumeric,
             col = day,
             y = fitness))+
  geom_jitter()+
