...
spearmanRes=spearmanRes%>%
  filter(Gene %in% cluster1Expresssion$SYMBOL)
...
'30,0447 mutant-gene pairs'
positiveRelationship = sigRes %>%
  filter(Spearman_Correlation > 0)
'12,699 negative mutant-gene pairs'
...
sigRes%>%
  ...
  ComplexHeatmap::Heatmap(show_row_names = F, show_column_names = F)
...
p=annotatedNegativeRelationship%>%
  ...
  geom_bar()+
  xlim(0,100)+
  labs(title = 'Cluster 1 negative association mutant functions')

ggsave(p,
       file = 'colonicRNAClusterIntegration/spearmanRes/cluster1NegativeAssociationMutantFunctions.pdf',
       height = 20,
       width = 20)

p=annotatedPositiveRelationship=positiveRelationship%>%
  rename(locusId = Mutant)%>%
  left_join(keggs, by = 'locusId')%>%
  select(locusId,kofamAccession) %>%
  distinct()%>%
...
p=annotatedPositiveRelationship%>%
  ...
  geom_bar()+
  xlim(0, 100)+
  labs(title = 'Cluster 1 positive association mutant functions')

ggsave(p,
       file = 'colonicRNAClusterIntegration/spearmanRes/cluster1PositiveAssociationMutantFunctions.pdf',
       height = 20,
       width = 20)